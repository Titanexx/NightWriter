...
func GetAttachmentsIDByOwner(ownerID uint, ownerType string) (attachments []Attachment) {
	config.DB.Model(&Attachment{}).Select("ID").Where("owner_id = ? and owner_type = ?", ownerID, ownerType).Find(&attachments)
	return attachments
}

func GetAttachmentByIDAndOwner(attachmentID uint, ownerID uint, ownerType string) (attachment Attachment) {
	config.DB.Model(&Attachment{}).Where("ID = ? and owner_id = ? and owner_type = ?", attachmentID, ownerID, ownerType).Find(&attachment)
	return attachment
}

func DeleteAttachmentsByIDs(ids []uint, ownerID uint, ownerType string) (err error) {
	err = config.DB.Where("ID in ? and owner_id = ? and owner_type = ?", ids, ownerID, ownerType).Delete(&Attachment{}).Error
	return err
}
