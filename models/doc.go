...
func IsDocAndPartExistByID(user User, docID uint, partID uint) bool {
	var count int64
	config.DB.Debug().Joins("JOIN user_docs ON user_docs.doc_id = parts.doc_id and user_docs.user_id = ?", user.ID).Model(&Part{}).Where("parts.doc_id = ? and ID = ?", docID, partID).Count(&count)
	return count == 1
}

func HasRightByDocIDAndUser(user User, docID uint, right DocRight) bool {
	var count int64
	config.DB.Debug().Model(&UserDoc{}).Where("user_id = ? and doc_id = ? and user_docs.right >= ?", user.ID, docID, right).Count(&count)
	return count == 1
}
...
func (doc *Doc) GetUsers() (users []User, err error) {
	err = config.DB.Debug().Model(&doc).Select("*").Association("Users").Find(&users)
...
func (doc *Doc) AddUser(user User, key string, right DocRight) (err error) {
	return config.DB.Debug().Create(&UserDoc{
		UserID: user.ID,
		DocID:  doc.ID,
		Key:    key,
		Right:  right,
...
func (doc *Doc) DeleteUser(user User) (err error) {
	return config.DB.Debug().Model(&doc).Association("Users").Delete(&user)
}
...
func (doc *Doc) GetParts() (parts []Part, err error) {
	err = config.DB.Debug().Model(&doc).Preload("Title").Preload("Characteristics").Preload("Content").Association("Parts").Find(&parts)
	return parts, err
}

func (doc *Doc) AddPart(part Part) (err error) {
	return config.DB.Debug().Model(&doc).Association("Parts").Append(&part)
}

func (doc *Doc) GetPart(partID uint) (part Part, err error) {
	err = config.DB.Debug().Model(&doc).Where("ID = ?", partID).Association("Parts").Find(&part)
	return part, err
}
...
func (doc *Doc) AddAttachment(attachment *Attachment) (err error) {
	return config.DB.Debug().Model(&doc).Association("Attachments").Append(&attachment)
}
